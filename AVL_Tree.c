...
AVLTREE *insertToAVL(int x, AVLTREE *tree) {
	if(tree!=NULL)
	{
		...
		if((height(tree->left)-height(tree->right)) > 1 && x< tree->left->data) // && sonrasýnda gelen veri en sola yerlesirse saga cevrilir left left dengesizligi
		 return RightRotate(tree);

		if((height(tree->left)-height(tree->right)) > 1 && x> tree->left->data) // sol sag degngesizligi    				 15
